...
export default function Footer() {
  return (
    <footer className="p-3 md:pt-8 mt-4 relative bg-(--background) border-t border-slate-200 dark:border-slate-800 overflow-hidden">
      {/* العلامة المائية — pointer-events-none + aria-hidden */}
      <div
        aria-hidden="true"
        className="absolute left-4 md:left-10 bottom-4 text- md:text- font-black text-(--accent) opacity-[0.03] select-none pointer-events-none tracking-tighter leading-none italic"
      >
        EGY PYRAMID
      </div>
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse, rgba(202,138,4,0.06), transparent 70%)",
        }}
      />
      ...
      <div className="max-w-350 mx-auto relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-6 gap-fluid-gap mb-3 md:mb-20">
          ...
          <div className="lg:col-span-3 space-y-8">
            ...
            <div className="flex flex-wrap gap-1 md:gap-4">
              {[
                ...
                {
                  Icon: Facebook,
                  href: "https://www.facebook.com/people/Egypyramid/61590525984381/",
                  label: "Facebook",
                },
                {
                  Icon: Youtube,
                  href: "https://www.youtube.com/@Egy-Pyramid",
                  label: "YouTube",
                },
                ...
                <Link
                  ...
                  aria-label={label}
                  className="w-12 h-12 flex items-center justify-center rounded-2xl border border-slate-300 dark:border-slate-700 text-slate-600 dark:text-slate-400 lg:hover:bg-amber-800 dark:hover:bg-(--accent) lg:hover:text-white dark:hover:text-slate-950 lg:hover:border-transparent transition-colors duration-200"
                >
                  <Icon size={18} />
                </Link>
              ))}
            ...
            <ul className="space-y-4 text-fluid-xs font-bold px-1 list-none">
              {[
                ...
                <li key={item.name}>
                  ...
                  >
                    <span className="w-1.5 h-1.5 bg-amber-800 dark:bg-(--accent) rounded-full shrink-0 opacity-60" />
                    {item.name}
                  </Link>
                </li>
              ))}
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <p className="text-slate-600 dark:text-slate-400 text-[12px] font-medium leading-relaxed">
              انضم لجروبنا على تليجرام لطلب الأفلام، متابعة التحديثات، والتواصل
              مع آلاف المتابعين.
            </p>
            ...
            >
              <div className="flex items-center gap-3 z-10">
                <div className="bg-amber-800 dark:bg-(--accent) p-2 rounded-xl text-white dark:text-slate-950">
                  <Send size={20} />
                </div>
                <div className="flex flex-col">
                  ...
                  <span className="text-(--foreground ) dark:text-slate-500 text-[10px]">
                    عبر مجتمعنا على Telegram
                  </span>
                </div>
              </div>
              <div className="z-10 bg-slate-200 dark:bg-slate-700/50 px-3 py-1 rounded-lg text- font-black text-slate-600 dark:text-slate-400">
                انضمام
              </div>
            </a>
          </div>
        ...
        <div className="pt-3 md:pt-10 border-t border-slate-200 dark:border-slate-800 flex flex-col md:flex-row justify-between items-center gap-6">
          <p className="text-slate-600 dark:text-slate-400 text-fluid-xs font-black tracking-widest uppercase antialiased">
            © 2026 Crafted with{" "}
            <Heart size={14} className="inline text-red-500 mx-1" /> by{" "}
            {/* ✅ amber-800 في اللايت بدل accent مباشرة */}
            <span className="text-amber-800 dark:text-(--accent)">
              EGY PYRAMID
            </span>
          </p>
