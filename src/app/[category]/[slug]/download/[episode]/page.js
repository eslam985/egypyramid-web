// /media/es/DDrive/projects/web-Next/egypyramid-web/src/app/[category]/[slug]/download/[episode]/page.js
...
// نظام الكاش لمنع تكرار جلب البيانات بين السيو وجسم الصفحة
const getEpisodeData = cache(async (mediaId, category, episode) => {
  return await mediaService.getEpisodeWithoutSeason(mediaId, category, episode);
});

export async function generateMetadata({ params }) {
  const { category, slug: encodedSlug, episode } = await params;
  ...
  const { data: epData } = await getEpisodeData(mediaId, category, episode);
  if (!epData)
  ...
  const dynamicTitle = `مشاهدة ${mediaTitle} بجودة عالية | ايجي بيراميد - EGY PYRAMID`;
...
export default async function DownloadPage({ params }) {
  ...
  // جلب البيانات من الكاش مباشرة باستخدام السيرفس
